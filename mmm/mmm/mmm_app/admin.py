...
class ProjectBaseAdmin(admin.ModelAdmin):
	def org_name(self, obj):
		u = UserInfo.objects.get(user=obj.sponsor)
		return u.org_name
...
class UserInfoAdmin(userBaseAdmin):
	fieldsets = (
		...
		('User Information', {
			'fields': ('major', 'bio', 'org_name', 'org_bio', 'image',)
		}),
		('Bookmarked Projects', {
			'fields': ('bookmarks',)
		}),
...
# Project
class ProjectAdmin(ProjectBaseAdmin):
	fieldsets = (
		('Project Information', {
			'fields':('title', 'image', 'org_name', 'sponsor', 'status', 'description', 'category_subs', 'show_in_gallery', 'flags',)
	...
	search_fields = ['sponsor__username', 'sponsor__email', 'sponsor__first_name', 'sponsor__last_name', 'title', 'category_subs__name', 'org_name']
	
	readonly_fields = ['org_name']
